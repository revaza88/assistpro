...
export default function LegalServicePage() {
  ...
  const benefits = [
    t('services.legal.benefitRiskPrevention'),
    t('services.legal.benefitCompliance'),
    t('services.legal.benefitInterestProtection'),
    t('services.legal.benefitOptimalSolutions'),
    t('services.legal.benefitResourceEfficiency'),
  ].filter(Boolean);

  const serviceList = (t('services.legal.serviceList', { returnObjects: true }) as string[] || []).filter(Boolean);
  const targetList = (t('services.legal.targetList', { returnObjects: true }) as string[] || []).filter(Boolean);
  const whyUsReasons = [
    t('services.legal.reasonExperience'),
    t('services.legal.reasonBusinessOriented'),
    t('services.legal.reasonComprehensiveVision'),
    t('services.legal.reasonProactivity'),
  ].filter(Boolean);
      ...
      <main className="flex-grow">
        ...
        >
          <div className="container mx-auto px-4 text-center">
            ...
            <ScaleIcon className="w-16 h-16 sm:w-20 sm:h-20 text-brand-gold-400 mx-auto mb-6" />
          ...
          >
            {benefits.length > 0 && (
              <section className="mb-10 sm:mb-12 lg:mb-16">
                <SectionTitle title={t('services.legal.whyImportantTitle')} icon={ShieldCheckIcon} />
            ...
            {serviceList.length > 0 && (
              <section className="mb-10 sm:mb-12 lg:mb-16">
                <SectionTitle title={t('services.legal.servicesIncludeTitle')} icon={DocumentTextIcon} />
            ...
            {targetList.length > 0 && (
                <section className="mb-10 sm:mb-12 lg:mb-16">
                    <SectionTitle title={t('services.legal.forWhomTitle')} icon={UserGroupIcon}/>
            ...
            {whyUsReasons.length > 0 && (
              <section className="mb-10 sm:mb-12 lg:mb-16">
                <SectionTitle title={t('services.legal.whyUsTitle')} icon={ArrowTrendingUpIcon} />
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button size="lg" asChild className="bg-brand-gold-500 hover:bg-brand-gold-600 text-brand-blue-700 font-semibold shadow hover:shadow-lg text-base">
                  <Link href="/contact">
                    {t('services.legal.ctaRequestConsultation')}
                  </Link>
                </Button>
